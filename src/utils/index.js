...
function getDAIAddress() {
  switch (localStorage.getItem("uniswap.network")) {
    ...
    case "4":
      return "0x2448eE2641d78CC42D7AD76498917359D961A783";
    default:
...
export function amountFormatter(
  ...
  // amount > 0
    ...
    const baseAmount = ethers.BigNumber.from(10).pow(
      ethers.BigNumber.from(baseDecimals)
    );
  ...
  if (inputValue && inputDecimals && outputValue && outputDecimals) {
    ...
    if (invert) {
      return inputValue
        ...
        .div(outputValue)
        .mul(
          ethers.BigNumber.from(10).pow(ethers.BigNumber.from(outputDecimals))
        )
        .div(
          ethers.BigNumber.from(10).pow(ethers.BigNumber.from(inputDecimals))
        );
    } else {
      return outputValue
        .mul(factor)
...
export function calculateGasMargin(value) {
  const offset = value.mul(GAS_MARGIN).div(ethers.BigNumber.from(10000));
...
export function USDToEth(USDExchangeRateETH, amount) {
  if (USDExchangeRateETH) {
    return amount
      ...
      .mul(ethers.BigNumber.from(10).pow(ethers.BigNumber.from(18)))
      .div(ethers.BigNumber.from(10).pow(ethers.BigNumber.from(2)))
